#!/bin/bash

# 简化版完全自动化部署脚本
# 只需要 GitHub Token，自动完成所有配置

set -e

echo "🚀 Cloudflare Pages 完全自动化部署"
echo "=================================="
echo ""
echo "🔑 只需要提供："
echo "   GitHub Personal Access Token (用于设置GitHub Secrets)"
echo ""
echo "📋 其他配置已自动包含："
echo "   - Cloudflare API Token: ThifwmItAHG3ZqgAIsaDsXEmT5--sXugBufl1-du"
echo "   - Cloudflare Account ID: e60142483b7847e196fdc287602ee06f"
echo ""
read -p "请输入你的 GitHub Personal Access Token: " GITHUB_TOKEN

if [ -z "$GITHUB_TOKEN" ]; then
    echo "❌ 错误：GitHub Token 不能为空"
    exit 1
fi

# 配置信息
GITHUB_REPO="lekaihuai2017/ai-background-remover"
CLOUDFLARE_API_TOKEN="ThifwmItAHG3ZqgAIsaDsXEmT5--sXugBufl1-du"
CLOUDFLARE_ACCOUNT_ID="e60142483b7847e196fdc287602ee06f"

echo ""
echo "🔍 验证 GitHub Token..."
if curl -s -H "Authorization: token $GITHUB_TOKEN" https://api.github.com/user | grep -q '"login"'; then
    echo "✅ GitHub Token 有效"
else
    echo "❌ GitHub Token 无效，请检查token"
    exit 1
fi

# 1. 推送代码（如果有未提交的更改）
echo "📁 检查并推送代码..."
if [ -n "$(git status --porcelain)" ]; then
    git add . && git commit -m "feat: 自动化部署准备"
fi
git push origin main
echo "✅ 代码已推送"

# 2. 设置GitHub Secrets
echo ""
echo "🔐 设置GitHub Secrets..."

# 获取Public Key
public_key_response=$(curl -s \
  -H "Authorization: token $GITHUB_TOKEN" \
  -H "Accept: application/vnd.github.v3+json" \
  "https://api.github.com/repos/$GITHUB_REPO/actions/secrets/public_key")

key_id=$(echo "$public_key_response" | grep -o '"key_id":"[^"]*"' | cut -d'"' -f4)
public_key=$(echo "$public_key_response" | grep -o '"public_key":"[^"]*"' | cut -d'"' -f4 | sed 's/\\n/\n/g')

# 加密并设置Secrets
encrypt_and_set() {
    local secret_name="$1"
    local secret_value="$2"
    encrypted_value=$(echo "$secret_value" | openssl rsautl -encrypt -pubin -inkey <(echo "$public_key") -pkcs1 | base64 | tr -d '\n')
    
    response=$(curl -s -X PUT \
      -H "Authorization: token $GITHUB_TOKEN" \
      -H "Accept: application/vnd.github.v3+json" \
      -H "Content-Type: application/json" \
      -d "{\"encrypted_value\":\"$encrypted_value\",\"key_id\":\"$key_id\"}" \
      "https://api.github.com/repos/$GITHUB_REPO/actions/secrets/$secret_name")
    
    if echo "$response" | grep -q '"success":true'; then
        echo "✅ $secret_name 设置成功"
    else
        echo "❌ $secret_name 设置失败"
    fi
}

# 设置两个Secrets
encrypt_and_set "CLOUDFLARE_API_TOKEN" "$CLOUDFLARE_API_TOKEN"
encrypt_and_set "CLOUDFLARE_ACCOUNT_ID" "$CLOUDFLARE_ACCOUNT_ID"

# 3. 触发部署
echo ""
echo "🚀 触发部署..."
echo "feat: 自动化配置完成" > trigger-deploy.md
git add trigger-deploy.md
git commit -m "feat: 自动化配置完成"
git push origin main

# 4. 等待并检查部署
echo ""
echo "⏳ 等待部署完成... (最多5分钟)"
sleep 300

# 检查部署状态
echo ""
echo "📊 检查部署状态..."

# 检查GitHub Actions
if curl -s "https://api.github.com/repos/$GITHUB_REPO/actions/runs?branch=main" | grep -q '"conclusion":"success"'; then
    echo "✅ GitHub Actions 部署成功！"
else
    echo "⚠️  GitHub Actions 可能还在进行中"
fi

# 检查Cloudflare Pages
cf_response=$(curl -s "https://api.cloudflare.com/client/v4/accounts/$CLOUDFLARE_ACCOUNT_ID/pages/projects/ai-background-remover" \
  -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN")

if echo "$cf_response" | grep -q '"success":true'; then
    deployment_url=$(echo "$cf_response" | grep -o '"deployment_url":"[^"]*"' | cut -d'"' -f4)
    project_url=$(echo "$cf_response" | grep -o '"url":"[^"]*"' | cut -d'"' -f4)
    
    echo "✅ Cloudflare Pages 部署成功！"
    echo "🌐 部署URL: $deployment_url"
    echo "📋 管理页面: $project_url"
else
    echo "⚠️  Cloudflare Pages 可能还在创建中"
fi

# 清理
rm -f trigger-deploy.md

echo ""
echo "🎉 部署流程完成！"
echo ""
echo "📊 监控链接："
echo "   GitHub Actions: https://github.com/lekaihuai2017/ai-background-remover/actions"
echo "   Cloudflare Pages: https://dash.cloudflare.com/pages"
echo ""
echo "🔄 后续更新："
echo "   git add . && git commit -m '更新内容' && git push"
echo ""
echo "🎊 恭喜！你的AI背景移除工具已完全自动化部署！"